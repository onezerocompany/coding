...
#
# Syntax: ./node-debian.sh [install] [directory to install nvm] [nvm version] [node version to install (use "none" to skip)] [non-root user] [Update rc files flag] [install node-gyp deps]

INSTALL=${1:-"false"}
export NVM_DIR=${2:-"/usr/local/share/nvm"}
export NVM_VERSION=${3:-"0.38.0"}
export NODE_VERSION=${4:-"lts"}
USERNAME=${5:-"automatic"}
UPDATE_RC=${6:-"true"}
INSTALL_TOOLS_FOR_NODE_GYP="${7:-true}"
SCRIPT_FOLDER=${8:-"/root/scripts"}
...
if command -v bash > /dev/null; then
    ...
    command cat "$SCRIPT_FOLDER/nvm-scripts/nvm-load-bash" >> "/home/$USERNAME/.bashrc"
fi

# Install into zsh if possible
if command -v zsh > /dev/null; then
    echo "Installing nvm into zsh..."
    command cat "$SCRIPT_FOLDER/nvm-scripts/nvm-load-zsh" >> "/home/$USERNAME/.zshrc"
fi
